...
class AttentionFusion(nn.Module):
    ...
    def forward(self, vgg_feat, sam_feat):
        # 对齐特征尺寸
        if sam_feat.size()[-2:] != vgg_feat.size()[-2:]:
            sam_feat = F.interpolate(sam_feat, size=vgg_feat.shape[2:], mode='bilinear')

        # 生成注意力权重
        attn = self.attn(torch.cat([vgg_feat, sam_feat], dim=1))
        return vgg_feat * attn + sam_feat * (1 - attn)
...
class BIGate(nn.Module):
    ...
    def forward(self, feat_a, feat_b):
        shared = self.conv_shared(torch.cat([feat_a, feat_b], dim=1))
        gate_a = self.gate_a(shared)
        gate_b = self.gate_b(shared)

        out_a = feat_a * gate_b + shared
        out_b = feat_b * gate_a + shared
...
class ConV5(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = DWConv(in_channels , in_channels // 2,3,1,1)
        self.conv2 = DWConv(in_channels // 2 , in_channels // 4,3,1,1)
        self.conv3 = DWConv(in_channels // 4 , in_channels // 8,3,1,1)
        self.conv4 = DWConv(in_channels // 8 , in_channels // 8,3,1,1)
...
class FMENet(nn.Module):
    def __init__(self,):
        ...
        # self.fuse1 = SAMFusion(128, 128)
        # self.fuse2 = SAMFusion(256, 256)
        # self.fuse3 = SAMFusion(512, 512)
        # self.fuse4 = SAMFusion(512, 512)

        self.fuse1 = AttentionFusion(128)
        self.fuse2 = AttentionFusion(256)
        self.fuse3 = AttentionFusion(512)
        self.fuse4 = AttentionFusion(512)

        self.bigate1 = BIGate(128)  
        self.bigate2 = BIGate(256)  
        self.bigate3 = BIGate(512)  
        self.bigate4 = BIGate(512)  
        ...
        self.channel2 = BasicConv2d(512,256,3,1,1)
        self.channel1 = BasicConv2d(256,128,3,1,1)

        self.pool = BasicConv2d_P(512,512,3,1,1)
        self.pool_2 = BasicConv2d_2P(512,512,3,1,1)
        ...
        self.conv_reduce_4s = DWConv(512,256,3,1,1)
        ...
        self.decoder_module4 = DWConv(1024,512,3,1,1)
        self.decoder_module3 = DWConv(768,256,3,1,1)
        self.decoder_module2 = DWConv(384,128,3,1,1)
        self.decoder_module1 = DWConv(128,128,3,1,1)
    ...
    def forward(self,A,B):
        ...
        # print(layer4_A.shape) torch.Size([1, 512, 16, 16])
        ...
        SAM_A_4 = self.pool(self.SAM4(A))
        SAM_B_4 = self.pool(self.SAM4(A))

        SAM_A_3 = self.SAM3(A)
        SAM_B_3 = self.SAM3(B)
        # #
        SAM_A_2 = self.up_2(self.SAM2(A))
        SAM_B_2 = self.up_2(self.SAM2(B))

        SAM_A_1 = F.interpolate(self.SAM1(A), 128, mode='bilinear', align_corners=True)
        SAM_B_1 = F.interpolate(self.SAM1(B), 128, mode='bilinear', align_corners=True)
        ...
        layer1_A = self.fuse1(layer1_A, SAM_A_1)
        layer1_B = self.fuse1(layer1_B, SAM_B_1)

        layer2_A = self.fuse2(layer2_A, SAM_A_2)
        layer2_B = self.fuse2(layer2_B, SAM_B_2)

        layer3_A = self.fuse3(layer3_A, SAM_A_3)
        layer3_B = self.fuse3(layer3_B, SAM_B_3)

        layer4_A = self.fuse4(layer4_A, SAM_A_4)
        layer4_B = self.fuse4(layer4_B, SAM_B_4)

        layer1_A, layer1_B = self.bigate1(layer1_A, layer1_B)  
        layer2_A, layer2_B = self.bigate2(layer2_A, layer2_B)  
        layer3_A, layer3_B = self.bigate3(layer3_A, layer3_B)  
        layer4_A, layer4_B = self.bigate4(layer4_A, layer4_B)  
