...
class FeatureFusionModule(nn.Module):
    def __init__(self, fuse_d, id_d, out_d):
        ...
        self.dw = DWConv(self.fuse_d, self.out_d, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, c_fuse, c):
        c_fuse = self.dw(c_fuse)
        c_out = self.relu(c_fuse + self.conv_identity(c))
        c_out = self.conv_fuse(c_out)
...
class FMENet(nn.Module):
    ...
    def forward(self,A,B):
        ...
        t1_fea1, t1_fea2, t1_fea3, t1_fea4 = self.backbone.base_forward(A)  # 64, 128, 256, 512
        t2_fea1, t2_fea2, t2_fea3, t2_fea4 = self.backbone.base_forward(B)
        print(t1_fea1.shape, t1_fea2.shape, t1_fea3.shape, t1_fea4.shape)
        layer1_A, layer2_A, layer3_A, layer4_A = self.PHF(t1_fea1, t1_fea2, t1_fea3, t1_fea4)
        layer1_B, layer2_B, layer3_B, layer4_B = self.PHF(t2_fea1, t2_fea2, t2_fea3, t2_fea4)
